...
func ReadDataType(context *parsing.Context) parsing.EntityPtr {
	// Convert "constraints" list (1.0) to "validation" (2.0) before calling v1.2 reader
	...
	// TOSCA 1.0 doesn't support the "KeySchema" field (introduced in TOSCA 1.3)
	context.SetReadTag("KeySchema", "")
	// TOSCA 1.0 doesn't support the "EntrySchema" field (introduced in TOSCA 1.3)
	context.SetReadTag("EntrySchema", "")
	// TOSCA 1.0 doesn't support the "Units" field (introduced in TOSCA 2.0)
	context.SetReadTag("Units", "")
	// TOSCA 1.0 doesn't support the "Prefixes" field (introduced in TOSCA 2.0)
